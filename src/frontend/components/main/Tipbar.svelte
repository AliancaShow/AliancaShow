<script>
    ...
    const messages = {
        donate: "O AliançaShow é construído sobre o FreeShow, um projeto gratuito e de código aberto. Considere apoiar os desenvolvedores originais, para que eles sigam atendendo às necessidades técnicas da Igreja."
...
{#if !isClosed && !$isDev}
    <section class="toolbar" transition:slide={{ duration: 150 }}>
        <div class="text">
            ...
            {#if activeMessage === "donate"}
                <MaterialButton style="margin-left: 10px;padding: 2px 10px;background: linear-gradient(160deg, #f1d278 0%, #f21a27 40%, #9c1114 80%, #750b0b 100%) !important;" on:click={donate}>
...
<style>
    section {
        ...
        border-image: linear-gradient(160deg, #750b0b 0%, #9c1114 20%, #f21a27 60%, #f1d278 100%) 1;
    }

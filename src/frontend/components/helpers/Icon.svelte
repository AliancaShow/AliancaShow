<script lang="ts">
    ...
    let baseColor = gradientColor || (gradient ? "#f1d278" : "#f21a27")
    $: if ($themes[$theme]) updateBaseColor()
    function updateBaseColor() {
        if (gradientColor || gradient) return
        baseColor = $themes[$theme]?.colors?.secondary || "#f21a27"
    }
        ...
        {#if !white && colorMid && colorEnd && colorStart}
            <defs>
                <linearGradient id={gradientId} x1="0%" y1="0%" x2="100%" y2="100%">
                    ...
                    <stop offset="50%" stop-color="#f1d278" />
                    <stop offset="100%" stop-color="#750b0b" /> -->
